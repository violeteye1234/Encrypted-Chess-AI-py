...
class GameState:
    def __init__(self):
        """
        Creating a 8 by 8 chess board, in a list which contains black('b') and white('w') pieces of chess.
        'R' - Rook
        'Q' - queen
        'K' - King
        'N'- Knight
        ...
        #Check, Checkamte and Stalemate Handling
        ...
        #En-passsant moves
        self.enpassant = ()  # coordinates for the square where en-passant capture is possible
        self.enpassant_log = [self.enpassant]

        #Castling possibilites, moves and log 
        self.castle_rights = Castle_Rights(True, True, True, True)
        self.castle_rights_log = [Castle_Rights(self.castle_rights.white_king_side, self.castle_rights.black_king_side,
                                               self.castle_rights.white_queen_side, self.castle_rights.black_queen_side)]
    ...
    def makeMove(self, move):
        ...
        self.board[move.end_row][move.end_col] = move.piece_shifted
        self.move_log.append(move)  # log the move so we can undo it later
        self.white_to_move = not self.white_to_move  # Switch players
        # updating king's location if moved
        if move.piece_shifted == "wK":
            self.white_king_location = (move.end_row, move.end_col)
        elif move.piece_shifted == "bK":
            self.black_king_location = (move.end_row, move.end_col)

        # pawn promotion
        if move.promoted_pawn:
            self.board[move.end_row][move.end_col] = move.piece_shifted[0] + "Q"

        # enpassant move
        if move.en_passant:
            self.board[move.start_row][move.end_col] = "--"  # capturing the pawn

        # updating enpassant_possible variable
        if move.piece_shifted[1] == "p" and abs(move.start_row - move.end_row) == 2:  # only on 2 square pawn advance
        ...
        if move.castling:
            ...
            if move.end_col - move.start_col == 2:  
                # moving the rook to its new square
        ...
        #updating log if en-passant occurs
        self.enpassant_log.append(self.enpassant)

        # update castling rights - whenever it is a rook or king move
        self.update_castle_rights(move)
        self.castle_rights_log.append(Castle_Rights(self.castle_rights.white_king_side, self.castle_rights.black_king_side,
                                                   self.castle_rights.white_queen_side, self.castle_rights.black_queen_side))
    ...
    def undoMove(self):
        ...
        Undo the last move
        """
        # Making sure that there is a move to undo
        if len(self.move_log) != 0:
            #removing move from the list (Like Stack)
            move = self.move_log.pop()
            self.board[move.start_row][move.start_col] = move.piece_shifted
            self.board[move.end_row][move.end_col] = move.piece_captured

            #Swapping players
            self.white_to_move = not self.white_to_move  

            # Updating the king's position if needed
            if move.piece_shifted == "wK":
            ...
            self.castle_rights = self.castle_rights_log[-1]  # set the current castle rights to the last one in the list
    ...
    def update_castle_rights(self, move):
        ...
        if move.piece_shifted == 'wK':
            ...
            self.castle_rights.black_queen_side = False
            self.castle_rights.black_king_side = False
        elif move.piece_shifted == 'wR':
            if move.start_row == 7:
                if move.start_col == 0:  # left rook
                    self.castle_rights.white_queen_side = False
                elif move.start_col == 7:  # right rook
                    self.castle_rights.white_king_side = False
        elif move.piece_shifted == 'bR':
            if move.start_row == 0:
                if move.start_col == 0:  # left rook
                    self.castle_rights.black_queen_side = False
                elif move.start_col == 7:  # right rook
    ...
    def get_valid_moves(self):
        ...
        """
        #Creating a temporary caslting rights list
        temp_castle_rights = Castle_Rights(self.castle_rights.white_king_side, self.castle_rights.black_king_side,
                                          self.castle_rights.white_queen_side, self.castle_rights.black_queen_side)
        ...
        if self.in_check:
            ...
            if len(self.checks) == 1: 
                ...
                for i in range(len(moves) - 1, -1, -1):  # iterate through the list backwards when removing elements
                    if moves[i].piece_shifted[1] != "K":  # move doesn't move king so it must block or capture
    ...
    #Determining if theres a check
    def inCheck(self):
        if self.white_to_move:
            return self.sq_attacked(self.white_king_location[0], self.white_king_location[1])
        else:
            return self.sq_attacked(self.black_king_location[0], self.black_king_location[1])\
    

    #Square that is going to be attacked            
    def sq_attacked(self, row, col):
    ...
    #Getting all the moves possible without considering the Checks
    ...
    def getPawnMoves(self, row, col, moves):
        ...
        if col - 1 >= 0:  # capture to the left
            if not piece_pinned or pin_direction == (move_amount, -1):
                ...
                if (row + move_amount, col - 1) == self.enpassant:
    ...
    def getCastleMoves(self, row, col, moves):
        if self.sq_attacked(row, col):
            return  # can't castle while in check
        if (self.white_to_move and self.castle_rights.white_king_side) or (
                not self.white_to_move and self.castle_rights.black_king_side):
            self.get_kingside_castle_moves(row, col, moves)
        if (self.white_to_move and self.castle_rights.white_queen_side) or (
                not self.white_to_move and self.castle_rights.black_queen_side):
            self.get_queenside_castle_moves(row, col, moves)

    def get_kingside_castle_moves(self, row, col, moves):
        if self.board[row][col + 1] == '--' and self.board[row][col + 2] == '--':
            if not self.sq_attacked(row, col + 1) and not self.sq_attacked(row, col + 2):
...
class Move:
    ...
    def __init__(self, start_square, end_square, board, en_passant=False, castling=False):
        self.start_row = start_square[0]
        self.start_col = start_square[1]
        self.end_row = end_square[0]
        self.end_col = end_square[1]
        self.piece_shifted = board[self.start_row][self.start_col]
        self.piece_captured = board[self.end_row][self.end_col]
        # pawn promotion
        self.promoted_pawn = (self.piece_shifted == "wp" and self.end_row == 0) or (
                self.piece_shifted == "bp" and self.end_row == 7)
        # en passant
        self.en_passant = en_passant
        if self.en_passant:
            self.piece_captured = "wp" if self.piece_shifted == "bp" else "bp"
        # castle move
    ...
    def get_chess_notation(self):
        ...
        if self.piece_captured != "--":
            if self.piece_shifted[1] == "p":
                return self.get_rank_file(self.start_row, self.start_col)[0] + "x" + self.get_rank_file(self.end_row,
                                                                                                    self.end_col)
            else:
                return self.piece_shifted[1] + "x" + self.get_rank_file(self.end_row, self.end_col)
        else:
            if self.piece_shifted[1] == "p":
                return self.get_rank_file(self.end_row, self.end_col)
            else:
                return self.piece_shifted[1] + self.get_rank_file(self.end_row, self.end_col)
    ...
    def __str__(self):
        ...
        if self.piece_shifted[1] == "p":
            if self.is_capture:
                ...
                return end_square + "Q" if self.promoted_pawn else end_square

        move_string = self.piece_shifted[1]
        if self.is_capture:
            move_string += "x"
